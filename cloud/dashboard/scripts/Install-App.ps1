...
if (Test-Path -LiteralPath $existingLauncher) {
    ...
    Ok "preserved $LauncherName (with its existing credentials)"
} else {
    # No launcher to preserve. Write a template rather than nothing: a missing
    # file makes the Startup shortcut fail with a dialog, whereas this fails
    # loudly in launch.log with the exact remedy. Placeholders only - this
    # script must never invent or embed a password.
    @"
@echo off
REM Credentials are set here because the packaged app does not reliably
REM inherit User-scope environment variables when started by the Startup
REM shortcut -- it then connects anonymously and mosquitto refuses it.
REM
REM EDIT THE PASSWORD BELOW. This file was generated by Install-App.ps1, which
REM does not know it and must never store it in the repo.
set FPMS_MQTT_USERNAME=fpms
set FPMS_MQTT_PASSWORD=CHANGE_ME
set FPMS_MQTT_HOST=127.0.0.1
set FPMS_MQTT_PORT=1883
start "" "$Dest\$ExeName"
"@ | Set-Content -LiteralPath (Join-Path $staging $LauncherName) -Encoding ascii
    Warn "No existing $LauncherName was found, so a TEMPLATE was written."
