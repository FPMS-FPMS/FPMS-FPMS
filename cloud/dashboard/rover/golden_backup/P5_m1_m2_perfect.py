...
# =====================
GRID_RES = 100
ROUTE_CLEARANCE = 130    # PHASE5: was 70 — too tight for 230mm chassis
...
def planning_obstacles(pts, target_marker=None):
    ...
    import math as _m
    _oth=_p5_odom["theta"]; _orx=_p5_odom["x"]; _ory=_p5_odom["y"]
    for ko in keepouts:
        if "wx" in ko:
            _dx=ko["wx"]-_orx; _dy=ko["wy"]-_ory
            kx=_dx*_m.cos(-_oth)-_dy*_m.sin(-_oth)
            ky=_dx*_m.sin(-_oth)+_dy*_m.cos(-_oth)
        else:
            kx,ky=ko.get("x",0),ko.get("y",0)
        kr=ko.get("r",150)
...
def simplify_path(pts, epsilon=80):
...
_p5_last_segments = []   # stored outbound segments for return trip

_p5_retrace = []  # outbound (turn_deg, mm_driven) for return trip

def _p5_navdrive(bot, target, timeout=55.0):
    import math as _m
    _TKMM=_m.pi*70.0/1320.0; _DRV=26; _TRN=36; _COAST=0.85; _HZ=20
    _FSTOP=120; _KPH=10
    ...
    def _face_and_drive(tgt):
        for attempt in range(15):
            ...
            if tgt!="home" and mm>0:
                _p5_retrace.append((err if abs(err)>2 else 0, mm))
    ...
    if target == "home" and _p5_last_segments:
        # Retrace outbound path backwards using stored segments
        ret_segs = [(-td, dd) for td, dd in reversed(_p5_last_segments)]
        _p5_last_segments.clear()
        event(f"P5 HOME: retracing {len(ret_segs)} segments backwards","k")
        for i,(td,dd) in enumerate(ret_segs):
            event(f"  ret[{i}]: turn {td:+.0f}° drive {dd:.0f}mm backward","i")
        for i,(turn_deg,drive_mm) in enumerate(ret_segs):
            if stop_requested: _stp(); return False
            mk=marker_copy(target)
            if mk and mk.get("locked") and float(mk["dist"])<=_stop_at:
                _stp(); event(f"P5 ARRIVED {target.upper()} at ret[{i}]","k")
                with lock: S["route_path"]=[]
                return True
            event(f"P5 ret[{i}/{len(ret_segs)}]: turn {turn_deg:+.0f}° drive {drive_mm:.0f}mm bwd","i")
            if abs(turn_deg)>2: _spin(turn_deg)
            mm_done,reason=_fwd(drive_mm,target,rev=True)
            event(f"P5 ret[{i}] drove {mm_done:.0f}mm bwd reason={reason}","i")
            if reason=="ARRIVED": return True
            if reason=="STOP": return False
            time.sleep(0.3)
        event(f"P5 retrace done → final approach HOME","k")
    ...
    if target != "home":
        _p5_last_segments.clear()
        _p5_last_segments.extend(segments)
    ...
    # Record how far face_and_drive covers (for retrace)
    _mk_before = marker_copy(target)
    _dist_before = float(_mk_before["dist"]) if _mk_before and _mk_before.get("locked") else 0
    result = _face_and_drive(target)
    _mk_after = marker_copy(target)
    _dist_after = float(_mk_after["dist"]) if _mk_after and _mk_after.get("locked") else 0
    _covered = _dist_before - _dist_after
    if target != "home" and _covered > 20:
        _p5_retrace.append((0, _covered))
        event(f"P5 final approach covered {_covered:.0f}mm (recorded for retrace)","i")
    return result
...
def api_state():
    with lock:
        return jsonify({
            ...
            "odom": dict(_p5_odom),
            "obstacles": (lambda kos,th=_p5_odom["theta"],ox=_p5_odom["x"],oy=_p5_odom["y"]: [
                {"x": round((k["wx"]-ox)*__import__("math").cos(-th)-(k["wy"]-oy)*__import__("math").sin(-th),1),
                 "y": round((k["wx"]-ox)*__import__("math").sin(-th)+(k["wy"]-oy)*__import__("math").cos(-th),1),
                 "r": k.get("r",150)} if "wx" in k
                else {"x":k.get("x",0),"y":k.get("y",0),"r":k.get("r",150)}
                for k in kos])(list(S.get("obstacles",[]))),
...
B6_TARGET_STOP_RAW = 300
B6_HOME_STOP_RAW = 280
...
def api_add_obstacle():
    ...
    with lock:
        th = _p5_odom["theta"]
        rx,ry = _p5_odom["x"],_p5_odom["y"]
        import math as _am
        wx = rx + (x*_am.cos(th) - y*_am.sin(th))
        wy = ry + (x*_am.sin(th) + y*_am.cos(th))
        S['obstacles'].append({"wx": wx, "wy": wy, "r": r})
        n = len(S['obstacles'])
...
def p5_run_mission_v2(initial_target):
    ...
    mission_active = True; stop_requested = False
    auto_cooldown_until = time.time() + AUTO_COOLDOWN_S
    _retrace_stack.clear(); _mission_queue.clear()
    ...
    try:
        ...
        mission_active = False; _retrace_stack.clear(); _mission_queue.clear()
        auto_cooldown_until = time.time() + AUTO_COOLDOWN_S
