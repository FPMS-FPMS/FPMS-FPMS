...
PAGE = r'''
<!doctype html><html><head><meta charset="utf-8"><title>FPMS B8.6 HOME TUNE</title>
<style>
...
<body><div class="top"><div><h1>FPMS B8.6 HOME TUNE</h1><div class="sub">200cm × 300cm centered map • one webserver • M1/M2=20cm • HOME≈7cm • repeat auto</div></div><div id="clock"></div></div>
...
# ============================================================
# FPMS B8.6 HOME TUNE MODE - 2026-05-06
...
# ============================================================
# FPMS B8.6 HOME TUNE SAFE MODE - 2026-05-06
...
# ============================================================
# END FPMS B8.6 HOME TUNE SAFE MODE
# ============================================================
...
# ============================================================
# FPMS B8.6 HOME TUNE PRECISION - 2026-05-06
...
# ============================================================
# END FPMS B8.6 HOME TUNE PRECISION
# ============================================================
...
# ============================================================
# FPMS B8.5 - 20CM TARGET GAP + REPEAT AUTO - 2026-05-06
# Overrides B8.4 auto behavior and M1/M2 gap only.
# Keeps HOME at 10cm and spray at 2s.
# ============================================================

# REQUIRED CHANGE:
# M1/M2 stop 20cm from front/top of car.
B84_TARGET_STOP_GAP_MM = 200

# Backup distance: LiDAR is ~200mm behind front, so 20cm front gap ~= 400mm raw dist.
B84_TARGET_STOP_DIST_BACKUP_MM = 410

# Slow/final zones adjusted so it does not creep too deep before stopping.
B84_TARGET_SLOW_GAP_MM = 430
B84_TARGET_FINAL_GAP_MM = 300

# Keep HOME unchanged at 10cm.
B84_HOME_STOP_GAP_MM = 100
B84_HOME_STOP_DIST_BACKUP_MM = 310

# Keep spray 2 seconds.
ESP32_SPRAY_MS = 2000

# AUTO repeat behavior:
# If alert stays active, run again after cooldown.
# This is useful for demo/testing where Node1 remains ALERT.
B85_AUTO_REPEAT_WHILE_ALERT = True
B85_AUTO_REPEAT_COOLDOWN_S = 9.0
B85_AUTO_FAILED_COOLDOWN_S = 3.0


def b85_alert_active_for_target(target):
    target = str(target).lower()
    with lock:
        if target == "m1":
            return bool(ESP.get("alert1"))
        if target == "m2":
            return bool(ESP.get("alert2"))
    return False
...
def choose_alert_target():
    """
    B8.5 AUTO chooser:
    - If ALERT1 remains active, M1 can run again after cooldown.
    - If ALERT2 remains active, M2 can run again after cooldown.
    - Chooses closest active locked target.
    """
    with lock:
        a1 = bool(ESP.get("alert1"))
        a2 = bool(ESP.get("alert2"))
...
def auto_loop():
    """
    B8.5 AUTO:
    - Auto runs when ROBOT ON + AUTO ON + not currently in mission.
    - It repeats if alert remains active, after cooldown.
    - It does not require alert to clear.
    """
    global auto_cooldown_until

    last_auto_target = None
    last_auto_started_at = 0.0
    ...
    while True:
        ...
        try:
            with lock:
                ready = bool(robot_enabled) and bool(auto_enabled) and not bool(mission_active)
                cooldown_until = float(auto_cooldown_until)
            ...
            # Extra cooldown guard for same still-active alert.
            if target == last_auto_target and now - last_auto_started_at < B85_AUTO_REPEAT_COOLDOWN_S:
                continue

            event(f"B8.5 AUTO ALERT -> {target.upper()} repeat_allowed={B85_AUTO_REPEAT_WHILE_ALERT}", "w")

            ok, msg = start_mission(target)

            if ok:
                last_auto_target = target
                last_auto_started_at = now
                auto_cooldown_until = time.time() + B85_AUTO_REPEAT_COOLDOWN_S
            else:
                event("B8.5 AUTO start failed: " + msg, "e")
                auto_cooldown_until = time.time() + B85_AUTO_FAILED_COOLDOWN_S

        except Exception as e:
            event("B8.5 auto_loop error: " + str(e), "e")
            time.sleep(0.5)


# Make page title obvious.
# ============================================================
# END FPMS B8.5
# ============================================================
...
# ============================================================
# FPMS B8.6 - HOME ONLY CLOSER + FASTER - 2026-05-06
# DO NOT TOUCH M1/M2.
# DO NOT TOUCH SPRAY.
# DO NOT TOUCH AUTO REPEAT.
# ============================================================

# HOME parking only:
# Previous HOME was stopping too far.
# New HOME target = about 7cm front gap.
B84_HOME_STOP_GAP_MM = 70

# Backup raw distance.
# LiDAR is ~200mm behind front, so 7cm front gap ~= 270mm raw distance.
B84_HOME_STOP_DIST_BACKUP_MM = 280

# Start slow/final later so it does not crawl too early.
B84_HOME_SLOW_GAP_MM = 330
B84_HOME_FINAL_GAP_MM = 170

# Make HOME return a bit faster/stronger.
B84_HOME_APPROACH_POWER = 36
B84_HOME_SLOW_POWER = 27
B84_HOME_FINAL_POWER = 20
B84_HOME_RECOVERY_POWER = 37

# Slightly longer HOME pulses, still safe.
B84_APPROACH_PULSE_S = 0.18
B84_SLOW_PULSE_S = 0.13
B84_FINAL_PULSE_S = 0.080
B84_SETTLE_S = 0.045

# Keep HOME re-face behavior.
B84_HOME_REFACE_ERR_DEG = 15
B84_HOME_REFACE_COOLDOWN_S = 1.1

# ============================================================
# END FPMS B8.6 HOME ONLY TUNE
# ============================================================


if __name__ == '__main__':
    event('FPMS B8.6 HOME TUNE starting', 'i')
