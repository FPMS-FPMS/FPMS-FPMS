...
# Stable ports
# =====================
D500_PORT = "/dev/serial/by-path/platform-fc800000.usb-usb-0:1.3:1.0-port0"
ESP_PORT = "/dev/serial/by-path/platform-xhci-hcd.11.auto-usb-0:1:1.0-port0"
YAHBOOM_PORT = "/dev/serial/by-path/platform-fc800000.usb-usb-0:1.2:1.0-port0"
...
# =====================
GRID_RES = 100
ROUTE_CLEARANCE = 60     # tight for 60cm wide arena — too tight for 230mm chassis
...
def scan_points(all_points=True):
    ...
    for deg, v in enumerate(sc):
        ...
        d, inten = v
        x, y = deg_xy(deg, d)
        if in_world(x, y) and -200 <= x <= 800 and -200 <= y <= 1100:
...
def planning_obstacles(pts, target_marker=None):
    ...
    for p in pts:
        ...
        if wx < -20 or wx > 620 or wy < -20 or wy > 1220:
            continue
...
def build_costmap(obs):
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            if wx < 0 or wx > 600 or wy < 0 or wy > 1200:
                occ[r][c] = True
    sc, sr = xy_cell(0, 0)
...
def metrics_loop():
    while True:
        ...
        for p in pts_full:
            ...
            if wx < 30 or wx > 570 or wy < 30 or wy > 1170:
                continue
...
_p5_odom = {"x": 500.0, "y": 50.0, "theta": 0.0}
def _p5_odom_reset():
    _p5_odom.update({"x": 500.0, "y": 50.0, "theta": 0.0})
...
def _p5_navdrive(bot, target, timeout=55.0):
    import math as _m
    _TKMM=_m.pi*70.0/1320.0; _DRV=26; _TRN=50; _COAST=0.93; _HZ=20
    _FSTOP=120; _KPH=10
    ...
    def _spin(deg):
        """Spin and return ACTUAL degrees rotated (from gyro)."""
        if abs(deg)<2.0: return 0.0
        ...
        ig=0.0; lt=time.time()
        while ig<rad*_COAST:
            if stop_requested: _stp(); return 0.0
            now=time.time(); dt=now-lt; lt=now
            if 0<dt<0.5: ig+=abs(_gz())*dt
            time.sleep(1.0/_HZ)
        _stp()
        # Measure coast while robot settles
        coast_ig=0.0; _t0c=time.time(); _ltc=_t0c
        while time.time()-_t0c<0.25:
            now=time.time(); dt=now-_ltc; _ltc=now
            if 0<dt<0.5: coast_ig+=abs(_gz())*dt
            time.sleep(0.02)
        total=ig+coast_ig
        actual_deg=_m.degrees(total)*(1 if deg>0 else -1)
        event(f"  rotated {actual_deg:+.0f}° (tgt {deg:+.0f}°)","i")
        return actual_deg
...
def mission_worker(target):
    ...
    try:
        ...
        stop_bot(bot)
        # Straighten heading to 0° after HOME
        try:
            with _odom_lock:
                hdg = math.degrees(_p5_odom["theta"])
            if abs(hdg) > 5:
                event(f"Straightening heading: {hdg:.1f}° → 0°", "i")
                _spin = globals().get("_spin")
                if _spin:
                    _spin(-hdg)
                stop_bot(bot)
        except:
...
<div class="panel"><div class="h">WAYPOINTS (FIXED)</div><div class="card"><div class="small"><span style="color:#ff4b70">M1: (100, 800)</span> &bull; <span style="color:#ffbf00">M2: (500, 800)</span> &bull; <span style="color:#3af07a">HOME: (300, 50)</span></div></div><div class="grid2"><button id="b_obs" onclick="setMode('obstacle')" style="border-color:#ff4b70;color:#ff4b70;font-size:16px;padding:12px">+ OBSTACLE (or right-click map)</button><button class="danger" onclick="clearMarkers()">RESET ODOM</button></div>
...
B6_TARGET_STOP_RAW = 250
B6_HOME_STOP_RAW = 30  # bird-eye: tighter tolerance for odom-based return
