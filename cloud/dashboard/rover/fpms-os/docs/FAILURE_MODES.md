...
| **Model and console not in git** | `yolo26n-rk3588.rknn`, `fpms_yolo_npu.py`, `~/fpms_console/` exist only on the old Pi. Copy before relying on detection or the UI. |
| **Nothing here has run on the Orange Pi 5B** | the board has not arrived. |
