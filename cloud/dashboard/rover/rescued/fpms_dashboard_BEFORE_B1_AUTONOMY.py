...
AUTO_FORWARD_POWER = -18
AUTO_BACKWARD_POWER = 18
AUTO_TURN_POWER = 16

# Target raw distance from LiDAR.
# LiDAR is 200mm behind front tip, so 250mm raw = 50mm from robot front.
AUTO_TARGET_STOP_RAW = 250

AUTO_FACE_TOL_DEG = 9
AUTO_DRIVE_ANGLE_TOL_DEG = 16
AUTO_TIMEOUT_S = 35
AUTO_RETURN_HOME = True

# If robot turns wrong direction after chassis test, change this to -1.
AUTO_TURN_SIGN = 1
...
MISSION = {
    ...
    "finished_at": 0
        ...
        try:
            ...
            robot_text = "Yahboom raw set_motor forward test complete."
            event(robot_text, "k")
            return
        except Exception as e:
...
def mission_event(msg, typ="i"):
    global MISSION
    with lock:
        MISSION["message"] = msg
    event("[MISSION] " + msg, typ)
...
def current_front_raw():
    with lock:
        return int(S.get("front_raw", 0))

def mission_stop_bot(bot):
    try:
        bot.set_motor(0, 0, 0, 0)
    except Exception:
        pass

def mission_forward(bot):
    bot.set_motor(
        AUTO_FORWARD_POWER,
        AUTO_FORWARD_POWER,
        AUTO_FORWARD_POWER,
...
def mission_spin(bot, direction):
    # direction > 0 = one spin direction, direction < 0 = opposite.
    direction = 1 if direction > 0 else -1
    direction *= AUTO_TURN_SIGN

    if direction > 0:
        bot.set_motor(AUTO_TURN_POWER, AUTO_TURN_POWER, -AUTO_TURN_POWER, -AUTO_TURN_POWER)
    else:
        bot.set_motor(-AUTO_TURN_POWER, -AUTO_TURN_POWER, AUTO_TURN_POWER, AUTO_TURN_POWER)

def mission_open_robot():
    from Rosmaster_Lib import Rosmaster
    bot = Rosmaster(com=YAHBOOM_PORT)
...
def mission_face_marker(bot, target_name, timeout=10.0):
    global mission_stop_requested

    mission_event(f"Facing {target_name.upper()} using LiDAR angle + IMU guard.", "i")
    t0 = time.time()

    try:
        while time.time() - t0 < timeout:
            if mission_stop_requested:
                mission_stop_bot(bot)
                return False

            m = marker_copy(target_name)
            if not m or not m.get("locked"):
                mission_stop_bot(bot)
                mission_event(f"{target_name.upper()} marker lost while turning.", "e")
                return False

            angle = signed_angle(m["angle"])

            if abs(angle) <= AUTO_FACE_TOL_DEG:
                mission_stop_bot(bot)
                mission_event(f"Facing {target_name.upper()} angle={angle:.1f}°.", "k")
                return True

            # Positive angle = marker to one side; negative = other side.
            # If turn direction is wrong after real test, flip AUTO_TURN_SIGN.
            mission_spin(bot, 1 if angle > 0 else -1)
            time.sleep(0.06)

        mission_stop_bot(bot)
        mission_event(f"Turn timeout while facing {target_name.upper()}.", "e")
        return False

    finally:
        mission_stop_bot(bot)

def mission_drive_to_marker(bot, target_name, stop_raw=AUTO_TARGET_STOP_RAW, timeout=AUTO_TIMEOUT_S):
    global mission_stop_requested

    mission_event(f"Driving toward {target_name.upper()} until raw distance <= {stop_raw}mm.", "i")
    t0 = time.time()
    ...
    try:
        while time.time() - t0 < timeout:
            ...
            if not m or not m.get("locked"):
                mission_stop_bot(bot)
                mission_event(f"{target_name.upper()} marker lost while driving.", "e")
                return False
            ...
            with lock:
                MISSION["message"] = f"Driving to {target_name.upper()}: dist={dist:.0f}mm angle={angle:.1f}°"

            if dist <= stop_raw:
                mission_stop_bot(bot)
                mission_event(f"Reached {target_name.upper()} stop distance: {dist:.0f}mm.", "k")
                return True

            if abs(angle) > AUTO_DRIVE_ANGLE_TOL_DEG:
                mission_stop_bot(bot)
                if not mission_face_marker(bot, target_name, timeout=6.0):
                    return False
        ...
        mission_stop_bot(bot)
        mission_event(f"Drive timeout toward {target_name.upper()}.", "e")
        return False

    finally:
        mission_stop_bot(bot)

def mission_spray(bot):
    if not SPRAY_ENABLED:
        mission_event("Spray step reached. SPRAY is disabled until pump/MOSFET test is confirmed.", "w")
        time.sleep(1.0)
        return True

    # Placeholder for future pump control.
    # We need exact confirmed Yahboom servo/GPIO API before enabling pump.
    mission_event(f"Spraying for {SPRAY_SECONDS}s.", "w")
    time.sleep(SPRAY_SECONDS)
    mission_event("Spray complete.", "k")
    return True

def mission_worker(target_name):
    global mission_active, mission_stop_requested, auto_cooldown_until

    mission_active = True
    mission_stop_requested = False
    auto_cooldown_until = time.time() + 10

    with lock:
        MISSION["state"] = "RUNNING"
        MISSION["target"] = target_name.upper()
        MISSION["last_error"] = ""
    ...
    try:
        if not robot_mode:
            raise RuntimeError("Robot mode is OFF. Turn ROBOT MODE ON before autonomous run.")

        if target_name not in ["m1", "m2", "home"]:
            raise RuntimeError("Bad mission target.")

        target = marker_copy(target_name)
        if not target or not target.get("locked"):
            raise RuntimeError(f"{target_name.upper()} marker is not locked.")

        if target_name != "home":
            home = marker_copy("home")
            if AUTO_RETURN_HOME and (not home or not home.get("locked")):
                raise RuntimeError("HOME marker must be locked before mission.")

        bot = mission_open_robot()
        mission_stop_bot(bot)

        # Plan check before starting.
        try:
            res = plan_to(target_name)
            mission_event("Planner: " + res.get("msg", "no planner message"), "i" if res.get("ok") else "e")
            if not res.get("ok"):
                raise RuntimeError(res.get("msg", "No route found."))
        except Exception as e:
            mission_event(f"Planner warning/error: {e}", "w")

        if not mission_face_marker(bot, target_name):
            raise RuntimeError("Could not face target.")

        if not mission_drive_to_marker(bot, target_name):
            raise RuntimeError("Could not reach target stop distance.")

        if target_name != "home":
            mission_spray(bot)

            if AUTO_RETURN_HOME:
                if not mission_face_marker(bot, "home"):
                    raise RuntimeError("Could not face HOME.")

                if not mission_drive_to_marker(bot, "home"):
                    raise RuntimeError("Could not return HOME.")

        mission_stop_bot(bot)

        with lock:
            MISSION["state"] = "DONE"
            MISSION["message"] = "Mission complete."
            MISSION["finished_at"] = time.time()

        mission_event("Mission complete.", "k")
        ...
        with lock:
            ...
            MISSION["message"] = str(e)
            MISSION["finished_at"] = time.time()

        mission_event(f"Mission error: {e}", "e")

    finally:
        if bot:
            mission_stop_bot(bot)
        mission_active = False
        auto_cooldown_until = time.time() + 8

def start_mission(target_name):
    global mission_active, mission_stop_requested

    if mission_active:
        return False, "Mission already running."

    mission_stop_requested = False
    threading.Thread(target=mission_worker, args=(target_name,), daemon=True).start()
    return True, f"Mission started for {target_name.upper()}."
...
def auto_alert_monitor():
    ...
    while True:
        try:
            ...
            if auto_alerts_enabled and robot_mode and not mission_active and now >= auto_cooldown_until:
                if esp.get("alert1"):
                    ok, msg = start_mission("m1")
                    if ok:
                        mission_event("AUTO ALERT1 triggered Route M1.", "w")
                        auto_cooldown_until = now + 20

                elif esp.get("alert2"):
                    ok, msg = start_mission("m2")
                    if ok:
                        mission_event("AUTO ALERT2 triggered Route M2.", "w")
                        auto_cooldown_until = now + 20

        except Exception as e:
            event("[AUTO ALERT MONITOR] " + str(e), "e")
...
<div class="h" style="margin-top:12px">AUTONOMY B1</div>
<div class="grid2">
  <button class="green" onclick="runMission('m1')">RUN M1</button>
  <button class="green" onclick="runMission('m2')">RUN M2</button>
  <button class="yellow" onclick="runMission('home')">RUN HOME</button>
  <button class="danger" onclick="stopMission()">STOP MISSION</button>
</div>
<div class="grid2" style="margin-top:7px">
  <button id="autoAlertBtn" class="danger" onclick="toggleAutoAlerts()">AUTO ALERTS OFF</button>
  <button class="yellow" onclick="clearRoute()">CLEAR ROUTE</button>
</div>
<div id="missionMsg" class="msg">Mission status appears here.</div>
...
async function runMission(target){
  if(!confirm('Autonomous mission will move the robot. Wheels/floor area clear? Continue?'))return;
  const js=await(await fetch('/api/run_mission',{
    method:'POST',
    headers:{'Content-Type':'application/json'},
    body:JSON.stringify({target:target})
  })).json();
  if(!js.ok)alert(js.msg);
}
async function stopMission(){
  const js=await(await fetch('/api/stop_mission',{method:'POST'})).json();
  if(!js.ok)alert(js.msg);
}
async function toggleAutoAlerts(){
  const js=await(await fetch('/api/toggle_auto_alerts',{method:'POST'})).json();
  setAutoAlertButton(js.auto_alerts_enabled);
}
function setAutoAlertButton(on){
  const b=$('autoAlertBtn');
  if(!b)return;
  b.textContent=on?'AUTO ALERTS ON':'AUTO ALERTS OFF';
...
async function poll(){
  try{
    ...
    setRobot(!!s.robot_mode);
    setAutoAlertButton(!!s.auto_alerts_enabled);
    if(s.mission){
      $('missionMsg').textContent='Mission: '+s.mission.state+' | Target: '+s.mission.target+' | '+s.mission.message;
    }
markerLine('m1',s.marker_info.m1);markerLine('m2',s.marker_info.m2);markerLine('home',s.marker_info.home);events(s.events);drawDyn();
...
def api_state():
    ...
    out["mission"] = dict(MISSION)
    out["auto_alerts_enabled"] = auto_alerts_enabled
    out["mission_active"] = mission_active
    return jsonify(out)
...
@app.route("/api/run_mission", methods=["POST"])
def api_run_mission():
    data = request.get_json(force=True)
    target = str(data.get("target", "")).lower()

    if target not in ["m1", "m2", "home"]:
        return jsonify(ok=False, msg="Bad mission target.")
...
@app.route("/api/toggle_auto_alerts", methods=["POST"])
def api_toggle_auto_alerts():
    global auto_alerts_enabled, auto_cooldown_until
    auto_alerts_enabled = not auto_alerts_enabled
    auto_cooldown_until = time.time() + 2
    event("AUTO ALERTS ENABLED." if auto_alerts_enabled else "AUTO ALERTS DISABLED.", "w" if auto_alerts_enabled else "k")
    return jsonify(ok=True, auto_alerts_enabled=auto_alerts_enabled)
...
if __name__ == "__main__":
    ...
    threading.Thread(target=auto_alert_monitor, daemon=True).start()
